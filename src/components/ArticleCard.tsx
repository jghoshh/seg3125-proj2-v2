...
const ArticleCard = ({ id, title, author, datePublished, imageUrl, tags }: ArticleCardProps) => {
  return (
    <Center p={5} ml={5} mr={5} w="300px" borderRadius="10px" borderWidth="1px" boxShadow="xl" role="group">
      <VStack h="100%" justifyContent="space-between">
        <Link to={`/article/${id}`} aria-labelledby={`article-title-${id}`}>
          <Image mb={4} src={imageUrl} alt={`Image for ${title}`} width="300px" height="190px" objectFit="cover" borderRadius="10px" />
          ...
          <Text as="p" textAlign="center">By: {author} <br />Published: {datePublished} </Text>
        </Link>
      </VStack>
    </Center>
  )
